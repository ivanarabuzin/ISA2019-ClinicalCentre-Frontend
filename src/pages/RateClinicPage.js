...
import { rateClinic } from '../services/ClinicService';

class RateClinicPage extends FormComponent {
    ...
    submit() {
        ...
        rateClinic(this.props.clinic.id, this.state.data.rate).then(response => {

            if(!response.ok) {
                this.props.onFinish(null);
                this.props.enqueueSnackbar('Error rate clinic', { variant: 'error' });
                return;
            }

            this.props.enqueueSnackbar('Clinic rated', { variant: 'success' });
...
export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(RateClinicPage)));